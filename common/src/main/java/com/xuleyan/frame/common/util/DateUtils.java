...
import com.xuleyan.frame.common.enums.DateEnum;
 ...
 *
 * @author xuleyan
 * @version DateUtils.java, v 0.1 2021-08-22 8:17 下午
 */
public class DateUtils {
    ...
    public static long diffDay(String startDate, String endDate) {
        if (!ValidateUtils.isLegalDate(startDate, DateFormatEnum.YEAR_MONTH_DAY)) {
            throw new IllegalArgumentException("startDate 格式非法,只支持 yyyy-MM-dd");
        }

        if (!ValidateUtils.isLegalDate(endDate, DateFormatEnum.YEAR_MONTH_DAY)) {
            throw new IllegalArgumentException("endDate 格式非法,只支持 yyyy-MM-dd");
        }

        Date start = DateFormatUtils.parse(startDate, DateFormatEnum.YEAR_MONTH_DAY);
        Date end = DateFormatUtils.parse(endDate, DateFormatEnum.YEAR_MONTH_DAY);
     ...
     * @param endDate   结束时间
     * @return
     * @apiNote 计算时间查处理方式: yyyy-MM-dd 不会处理 HH:mm:ss
     */
    public static long diffDay(Date startDate, Date endDate) {
        LocalDate before = LocalDate.of(startDate.getYear() + 1900, Month.of(startDate.getMonth() + 1), startDate.getDate());
        LocalDate after = LocalDate.of(endDate.getYear() + 1900, Month.of(endDate.getMonth() + 1), endDate.getDate());
     ...
     * @return
     */
    public static Date getDawnDate(Date date) {
